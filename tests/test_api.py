...
class APITestCase(unittest.TestCase):
    ...
    def test_bad_auth(self):
        ...
        u = User(email='gon@example.com', password='dog', confirmed=True,
                 role=r)
        db.session.add(u)
        db.session.commit()
        ...
        response = self.client.get(
            '/api/v1/posts/',
            headers=self.get_api_headers('gon@example.com', 'cat'))
        self.assertEqual(response.status_code, 401)
    ...
    def test_token_auth(self):
        ...
        response = self.client.get(
            '/api/v1/token',
            headers=self.get_api_headers('gon@example.com', 'dog'))
    ...
    def test_unconfirmed_account(self):
        ...
        r = Role.query.filter_by(name='User').first()
        self.assertIsNotNone(r)
        u = User(email='gon@example.com', password='dog', confirmed=False,
                 role=r)
        db.session.add(u)
        db.session.commit()
    ...
    def test_users(self):
        ...
        r = Role.query.filter_by(name='User').first()
        self.assertIsNotNone(r)
        u1 = User(email='gon@example.com', username='gon',
                  password='dog', confirmed=True, role=r)
        u2 = User(email='kirua@example.com', username='kirua',
                  password='fox', confirmed=True, role=r)
        ...
        response = self.client.get(
            ...
            headers=self.get_api_headers('kirua@example.com', 'fox'))
        self.assertEqual(response.status_code, 200)
        json_response = json.loads(response.get_data(as_text=True))
        self.assertEqual(json_response['username'], 'gon')
        response = self.client.get(
            '/api/v1/users/{}'.format(u2.id),
            headers=self.get_api_headers('kirua@example.com', 'fox'))
        self.assertEqual(response.status_code, 200)
        json_response = json.loads(response.get_data(as_text=True))
        self.assertEqual(json_response['username'], 'kirua')
    ...
    def test_comments(self):
        ...
        self.assertEqual(response.status_code, 200)
        json_response = json.loads(response.get_data(as_text=True))
        self.assertEqual(json_response['body'], 'Thank you!')
